...
export function ProjectCard({ p }: { p: Project }) {
  ...
  const imgSrc = projectImageSrc(p.title);

  return (
    <article className="group relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-b from-white/10 to-white/5 transition duration-300 hover:-translate-y-1 hover:border-white/25 hover:shadow-[0_0_0_1px_rgba(255,255,255,0.08),0_20px_60px_rgba(0,0,0,0.35)]">
      ...
      <div className="relative">
        <div className="relative h-48 w-full overflow-hidden border-b border-white/10 bg-black/30 sm:h-56">
          {imgSrc ? (
            <img
              src={imgSrc}
              alt={`${p.title} preview`}
              ...
              onError={(e) => {
                // hide broken image if user didn't add screenshots yet
          ...
          <div className="absolute left-4 top-4 flex flex-wrap gap-2">
            ...
            {isShipped ? <Badge tone="success">Shipped</Badge> : null}

            {isInProgress ? <Badge>Coming Soon</Badge> : null}
            {hasLive ? <Badge>Live</Badge> : <Badge>No Demo</Badge>}
          </div>
        </div>
        ...
        <div className="relative p-8">
          <div className="flex flex-wrap items-start justify-between gap-4">
            ...
            {/* Small status summary on right (optional) */}
            <div className="hidden flex-wrap gap-2 sm:flex">
              {p.status ? (
                <Badge tone={isInProgress ? "warning" : isShipped ? "success" : "default"}>
                  {p.status}
                </Badge>
              ) : null}
            </div>
